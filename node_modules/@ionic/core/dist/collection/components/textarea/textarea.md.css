 ...
 */
:host(.textarea-fill-outline:not(.label-floating)) .textarea-control {
  position: relative;
}
...
:host(.textarea-fill-outline.textarea-label-placement-floating) .label-text-wrapper {
  ...
  position: absolute;
  /**
   * While resting this is the width of the textarea, which already
 ...
 * overlap the outline container while still
