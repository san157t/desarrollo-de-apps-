...
.textarea-control {
  ...
  width: 100%;
  /**
  * Allows the control to shrink below the label and textarea's intrinsic
  * width, preventing the start and end slots from being pushed outside
  * the wrapper. The label and textarea truncate independently, so it can
...
/**
 * When the label sits beside the textarea, the label should size to its content
 * and the textarea should take the remaining space. A zero flex basis prevents
 * the textarea from requesting its full width and forcing the label to shrink.
 ...
 */
:host(.textarea-label-placement-start) .native-wrapper,
:host(.textarea-label-placement-end) .native-wrapper,
:host(.textarea-label-placement-fixed) .native-wrapper {
  flex: 1 1 0;
}
...
.label-text-wrapper {
  ...
  * textarea and end slot outside the wrapper.
  */
  min-width: 0;
  /**
 ...
 * on the right in RTL.
 */
:host(.textarea-label-placement-start) .textarea-control {
  flex-direction: row;
}
 ...
 * on the left in RTL.
 */
:host(.textarea-label-placement-end) .textarea-control {
...
/**
 * Give stacked and floating textareas a taller minimum height to
 * accommodate the label and textarea content.
 */
:host(.textarea-label-placement-floating),
:host(.textarea-label-placement-stacked) {
  min-height: 56px;
...
:host(.textarea-label-placement-floating) ::slotted(ion-button[slot=start]),
:host(.textarea-label-placement-floating) ::slotted(ion-button[slot=end]),
:host(.textarea-label-placement-stacked) ::slotted(ion-button[slot=start]),
:host(.textarea-label-placement-stacked) ::slotted(ion-button[slot=end]) {
  margin-top: 10px;
}
