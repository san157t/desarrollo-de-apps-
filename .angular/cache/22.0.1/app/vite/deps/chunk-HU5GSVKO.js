...
// node_modules/@ionic/core/components/p-5ldEpNdG.js
...
var r2 = (o, s2, e2, r3, i2, l2, f, p, $) => {
  ...
  const g = a(r3).querySelector("ion-icon").getBoundingClientRect(), v = s2 ? g.width / 2 - (g.right - i2.right) + "px" : i2.left - g.width / 2 + "px", w = s2 ? `-${window.innerWidth - i2.right}px` : `${i2.left}px`, k = `${$.top}px`, T = `${i2.top}px`, A = e2 ? [{ offset: 0, transform: `translate3d(${w}, ${T}, 0)` }, { offset: 1, transform: `translate3d(${v}, ${k}, 0)` }] : [{ offset: 0, transform: `translate3d(${v}, ${k}, 0)` }, { offset: 1, transform: `translate3d(${w}, ${T}, 0)` }], j = e2 ? [{ offset: 0, opacity: 1, transform: h }, { offset: 1, opacity: 0, transform: x }] : [{ offset: 0, opacity: 0, transform: x }, { offset: 1, opacity: 1, transform: h }], B = e2 ? [{ offset: 0, opacity: 1, transform: "scale(1)" }, { offset: 0.2, opacity: 0, transform: "scale(0.6)" }, { offset: 1, opacity: 0, transform: "scale(0.6)" }] : [{ offset: 0, opacity: 0, transform: "scale(0.6)" }, { offset: 0.6, opacity: 0, transform: "scale(0.6)" }, { offset: 1, opacity: 1, transform: "scale(1)" }], I = r(), J = r(), N = r(), O = n("ion-back-button"), S = a(O).querySelector(".button-text"), V = a(O).querySelector("ion-icon");
  O.text = r3.text, O.mode = r3.mode, O.icon = r3.icon, O.color = r3.color, O.disabled = r3.disabled, O.style.setProperty("display", "block"), O.style.setProperty("position", "fixed"), J.addElement(V), I.addElement(S), N.addElement(O), N.beforeStyles({ position: "absolute", top: "0px", [u]: "0px" }).beforeAddWrite((() => {
    r3.style.setProperty("display", "none"), O.style.setProperty(b, d);
  })).afterAddWrite((() => {
    r3.style.setProperty("display", ""), O.style.setProperty("display", "none"), O.style.removeProperty(b);
  })).keyframes(A), I.beforeStyles({ "transform-origin": `${b} top` }).keyframes(j), J.beforeStyles({ "transform-origin": `${m} center` }).keyframes(B), o.addAnimation([I, J, N]);
};
  ...
  try {
    ...
    if (h || 0 !== v.length || 0 !== g.length ? (k.addElement(h), k.addElement(g)) : k.addElement(y.querySelector(":scope > .ion-page, :scope > ion-nav, :scope > ion-tabs")), w.addAnimation(k), x ? k.beforeClearStyles([f]).fromTo("transform", `translateX(${u})`, `translateX(${$})`).fromTo(f, d, 1) : k.beforeClearStyles([f]).fromTo("transform", `translateX(${m})`, `translateX(${$})`), h) {
      ...
      if (o) {
        ...
        s2.addElement(o).beforeStyles({ opacity: "1", display: "block" }).afterStyles({ opacity: "", display: "" }), e2.addElement(n3).beforeClearStyles([f]).fromTo(f, 0, 0.1), r3.addElement(a2).beforeClearStyles([f]).fromTo(f, 0.03, 0.7), s2.addAnimation([e2, r3]), k.addAnimation([s2]);
      }
    }
    ...
    if (v.forEach(((o) => {
      ...
      if (s2 || 0 !== e2.length || 0 !== r3.length ? (n3.addElement(s2), n3.addElement(r3)) : n3.addElement(X.querySelector(":scope > .ion-page, :scope > ion-nav, :scope > ion-tabs")), w.addAnimation(n3), x) {
      ...
      e2.forEach(((o) => {
        ...
        if (h2 && X2.addElement(h2), n4.addAnimation([s3, e3, d2, X2, y2]), w.addAnimation(n4), X2.fromTo(f, 0.99, 0), e3.fromTo(f, 0.99, 0), d2.fromTo(f, 0.99, 0), x) {
...
//# sourceMappingURL=chunk-HU5GSVKO.js.map
